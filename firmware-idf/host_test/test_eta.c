...
int main(void) {
    ...
    assert(eta_update(&st, -1, 480, 1000000) == 0);
    assert(eta_update(&st, 4000, 20, 1000000) == 0);         // taxiing
    assert(eta_update(&st, 4000, 480, 0) == 0);              // clock unknown

    // --- appears IMMEDIATELY from the instantaneous speed ---
    double now = 1000000, dist = 4000;
    long first = eta_update(&st, dist, 480, now + 1);
    ...
    for (int i = 1; i <= 2400; i++) {                        // 20 min of 0.5 s ticks
        ...
        dist -= g * 0.5 / 3600.0;
        long m = eta_update(&st, dist, g, now + t);
    ...
    for (int i = 1; i <= 2400; i++) {                        // 20 min at 430 kt
        double t = i * 0.5;
        dist -= 430 * 0.5 / 3600.0;
        m2 = eta_update(&st, dist, 430, t0 + t);
    }
    ...
    dist += 2000;
    long m3 = eta_update(&st, dist, 430, t0 + 1201);
    ...
    for (int i = 1; i <= 700; i++) {                         // rebuild some history
        double t = i * 0.5;
        dist -= 430 * 0.5 / 3600.0;
        m3 = eta_update(&st, dist, 430, t0 + 1201 + t);
    }
    assert(eta_update(&st, -1, 430, t0 + 1552) == 0);
    long m4 = eta_update(&st, dist, 430, t0 + 1553);
